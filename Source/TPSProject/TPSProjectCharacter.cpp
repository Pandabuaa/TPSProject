...
ATPSProjectCharacter::ATPSProjectCharacter()
{
	...
	Combat->SetIsNetStartupComponent(true);
...
void ATPSProjectCharacter::SetupPlayerInputComponent(class UInputComponent* PlayerInputComponent)
{
	...
	PlayerInputComponent->BindAction("Equip", IE_Pressed, this, &ATPSProjectCharacter::EquipButtonPressed);
	PlayerInputComponent->BindAction("Fire", IE_Pressed, this, &ATPSProjectCharacter::FireButtonPressed);
	PlayerInputComponent->BindAction("Fire", IE_Released, this, &ATPSProjectCharacter::FireButtonReleased);
...
void ATPSProjectCharacter::GetLifetimeReplicatedProps(TArray<FLifetimeProperty>& OutLifetimeProps) const
{
	Super::GetLifetimeReplicatedProps(OutLifetimeProps);
	DOREPLIFETIME(ATPSProjectCharacter, OverlappingWeapon);
	DOREPLIFETIME(ATPSProjectCharacter, Health);
}

void ATPSProjectCharacter::PostInitializeComponents()
{
...
{
	// If TPSProject is added to a project via 'Add Feature' in the Unreal Editor the dependency on HeadMountedDisplay in TPSProject.Build.cs is not automatically propagated
...
void ATPSProjectCharacter::EquipButtonPressed()
{
	if(Combat)
	{
...
bool ATPSProjectCharacter::IsWeaponEquipped()
{
	return (Combat && Combat->EquippedWeapon);
}

bool ATPSProjectCharacter::IsWeaponFire()
{
	return bFire;
}

void ATPSProjectCharacter::FireButtonPressed()
{
	 if(Combat)
	 {
		 Combat->FileButtonPressed(true);
		 bFire = true;
	 }
}

void ATPSProjectCharacter::FireButtonReleased()
{
	if (Combat)
	{
		Combat->FileButtonPressed(false);
		bFire = false;
	}
}

void ATPSProjectCharacter::PlayFireMontage()
{
	...
	if(AnimInstance && FireWeaponMontage)
	{
		...
		AnimInstance->Montage_JumpToSection(FName("RifleHip"));
	}
}

void ATPSProjectCharacter::OnRep_OverlappingWeapon()
{

}

void ATPSProjectCharacter::OnRep_Health()
{
